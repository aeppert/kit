// Package dogstatsd implements a DogStatsD backend for package metrics. It's
// very similar to statsd, but supports arbitrary tags per-metric, which map to
// Go kit's label values. So, while label values are no-ops in statsd, they are
...
//
//    s, stop := New("myprefix.", "udp", "dogstatsd:8125", time.Second, log.NewNopLogger())
//    defer stop()
//
...
// Dogstatsd is a collection of individual metrics. To send metrics to a
// DogStatsD server, create a Dogstatsd object, use it to create metrics
// objects, and pass those objects as dependencies to the components that will
// use them.
...
// their observations dumped and are reset.
...
// NewHistogram returns an empty DogStatsD histogram, ready for observations.
// Note that this probably isn't useful to you as a user; you probably want the
// NewHistogram method on the Dogstatsd type.
...
// NewSet returns a new DogStatsD set metric. Note that this probably isn't
// useful to you as a user; you probably want the NewSet method on the Dogstatsd
// type.
func NewSet() *Set {
	return &Set{
		values: map[string]struct{}{},
...
func (s *Set) Values() []string {
	...
	s.values = map[string]struct{}{} // TODO(pb): better garbage behavior possible?
	return res
}
